    ...
    for (const auto c : str) {
        switch (c) {
            ...
            case 'a':
                ...
                break;
            default:
                throw std::runtime_error{std::string{"Unknown object type '"} + c + "' (allowed are 'n', 'w', 'r', and 'a')."};
        }
    }
...
osmium::Box parse_bbox(const std::string& str, const std::string& option_name) {
    ...
    if (coordinates.size() != 4) {
        throw std::runtime_error{"Need exactly four coordinates in " + option_name + " option."};
    }
    ...
    if (!box.valid()) {
        throw std::runtime_error{"Invalid bounding box in " + option_name + " option. Format is LONG1,LAT1,LONG2,LAT2."};
    }
...
osmium::item_type parse_item_type(const std::string& t) {
    ...
    throw std::runtime_error{std::string{"Unknown default type '"} + t + "' (Allowed are 'node', 'way', and 'relation')."};
}
